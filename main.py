...
def create_rasters(output_path):
    ...
    for col in columns_to_check:
        ...
        text_box.insert("end", "Creating Raster: " + raster_filename + "\n")
        if os.path.exists(raster_filename):
            driver.DeleteDataSource(raster_filename)
        gdal.Grid(raster_filename, src_shp, zfield='value', format="GTiff", outputType=output_type, outputSRS="EPSG:4326", noData=nodata, algorithm= "invdist:power=3.0:smoothing=0.0006:radius1=1500:radius2=1500:angle=0.0:max_points=0:min_points=0:nodata=0.0")
...
upload_button = tk.Button(root, text="Upload Soil CSV", command=open_sensor_file)
